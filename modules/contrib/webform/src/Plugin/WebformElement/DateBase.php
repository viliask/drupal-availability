...
abstract class DateBase extends WebformElementBase {
  ...
  public static function validateDate(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    if (isset($element['#date_date_min'])) {
      ...
      if ($time < $min) {
        $form_state->setError($element, t('%name musi być po %min.', [
          '%name' => $name,
    ...
    if (isset($element['#date_date_max'])) {
      ...
      if ($time > $max) {
        $form_state->setError($element, t('%name musi by przed %max.', [
          '%name' => $name,
    ...
    if (isset($element['#date_min'])) {
      ...
      if ($time < $min) {
        $form_state->setError($element, t('%name musi być dokładnie lub po %min.', [
          '%name' => $name,
    ...
    if (isset($element['#date_max'])) {
      ...
      if ($time > $max) {
        $form_state->setError($element, t('%name musi być dokładnie lub przed %max.', [
          '%name' => $name,
    ...
    if (!empty($element['#date_days'])) {
      ...
      if (!in_array($day, $days)) {
        $form_state->setError($element, t('%name musi być %days.', [
          '%name' => $name,
